  ...
  return (
    <div className={`z-10 fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 ${isOpen ? 'block' : 'hidden'}`}>
      <div className="modal-content bg-white shadow-md p-4">
        <h2 className="text-lg font-semibold mb-2">이벤트 날짜 변경</h2>
        ...
        <label className="block mb-2">새로운 날짜:
          <input className="border rounded p-1 w-full" type="date" value={newDate} onChange={(e) => setNewDate(e.target.value)} />
        </label>
